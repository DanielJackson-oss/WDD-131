...
displayRecipes(sortByName(recipes));
...
function handleSearch() {
  ...
  const matches = recipes.filter((recipe) => recipeMatchesTerm(recipe, term));

  displayRecipes(sortByName(matches));
}
...
function sortByName(recipeList) {
...
function displayRecipes(recipeList) {
  ...
  if (recipeList.length === 0) {
    recipeContainer.innerHTML =
      '<p class="no-results">No recipes matched your search. Try a different keyword.</p>';
    return;
  }

  recipeList.forEach((recipe) => {
    recipeContainer.insertAdjacentHTML('beforeend', recipeCardTemplate(recipe));
  });
}
  ...
  return `
    <article class="recipe-card">
      ...
      <div class="recipe-details">
        ...
        ${starRatingTemplate(recipe.rating)}
